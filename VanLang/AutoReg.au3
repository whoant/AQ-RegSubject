...
_HttpRequest_SetRoot('https://online.vanlanguni.edu.vn')
...
Func _Find($find_what, $type = 0)
	...
	$http = _HttpRequest(2, "/ajaxpro/EduSoft.Web.UC.DangKyMonHoc,EduSoft.Web.ashx", $post, $_COOOKIE, '', 'X-AjaxPro-Method:LocTheoMonHoc')
...
Func _Check()
	$http = _HttpRequest(2, "/Default.aspx?page=dkmonhoc", '', $_COOOKIE)
	If @error Then
...
Func _Reg($m, $id, $th = '  ')
	...
	$http = _HttpRequest(2, "/ajaxpro/EduSoft.Web.UC.DangKyMonHoc,EduSoft.Web.ashx", $DangKySelectedChange, $_COOOKIE, '', 'X-AjaxPro-Method: DangKySelectedChange')
	...
	If Number($return[10]) == 0 Then
		...
		If $return[7] == "" And $return[8] == "" And $return[11] == "" Then
			...
			$a = _HttpRequest(1, "/ajaxpro/EduSoft.Web.UC.DangKyMonHoc,EduSoft.Web.ashx", $LuuVaoKetQuaDangKy, $_COOOKIE, '', 'X-AjaxPro-Method: LuuVaoKetQuaDangKy')
			$b = _HttpRequest(1, "/ajaxpro/EduSoft.Web.UC.DangKyMonHoc,EduSoft.Web.ashx", '{}', $_COOOKIE, '', 'X-AjaxPro-Method: KiemTraTrungNhom')
			$c = _HttpRequest(1, "/ajaxpro/EduSoft.Web.UC.DangKyMonHoc,EduSoft.Web.ashx", '{}', $_COOOKIE, '', 'X-AjaxPro-Method:LuuDanhSachDangKy')
			$d = _HttpRequest(2, "/ajaxpro/EduSoft.Web.UC.DangKyMonHoc,EduSoft.Web.ashx", '{"isCheckSongHanh":false,"ChiaHP":false}', $_COOOKIE, '', 'X-AjaxPro-Method: LuuDanhSachDangKy_HopLe')
			_HttpRequest_Test($d)
			If (StringInStr($d, 'error') == 0 AND StringInStr($d, 'Ngoài thời gian cho') == 0 ) Then
...
Func _Login($times = 0)
	...
	Local $aForm = [['__EVENTTARGET', ''], ['__EVENTARGUMENT', ''], ['ctl00$ContentPlaceHolder1$ctl00$ucDangNhap$txtTaiKhoa', GUICtrlRead($acc)], ['ctl00$ContentPlaceHolder1$ctl00$ucDangNhap$txtMatKhau', GUICtrlRead($pass)], ['ctl00$ContentPlaceHolder1$ctl00$ucDangNhap$btnDangNhap', 'Đăng Nhập']]
...
Func _Captcha($times = 0)
	...
	If @error Then
		_Text("[CAPTCHA] Không lấy được nội dung ")
		Return False
	EndIf
...
Func _Logout()
	...
	$http = _HttpRequest(1, "/default.aspx?page=gioithieu", $sDataToSend, $_COOOKIE)
